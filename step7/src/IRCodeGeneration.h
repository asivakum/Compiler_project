...
void PushIntoExprDeque(string symbol) {
    //cout << "[Debug] PushIntoExprDeque() is called, with " << symbol << " pushed in"  << endl;
...
void GenerateIRCode_Param_CallEXPR() {
    //cout << "[Debug] GenerateIRCode_Param_CallEXPR() is called" << endl;
 ...
 /// Ashiwan step6 - no paranthesized expression in function calls issue
    ...
    if(temp_expr_to_hold.size() == 1) {
      ...
      // Call GenerateFromPostOrder and get the result
       ...
       //cout << "push_ir_name: " << push_ir_name << endl;
...
void GenerateIRCode_RET() {
    ...
    else if(end_opcode == "ID") {
        string act_var = GetIRVarName(end_result);
        string id_type = SearchCurrFuncSymbolTableForType(end_result);
     //   cout << "Curr fun : " << currFuncName << " act_var: " << act_var << " id_type:" << id_type << endl;
        if(id_type == "FLOAT")
            temp1.opcode = "STOREF";
        else if (id_type == "INT")
            temp1.opcode = "STOREI";
...
void AddRETIfNotThere(string funcName) {
	if (FuncIRList.find(funcName) == FuncIRList.end()) {
		cerr << "[ERROR] Function name not found in FuncIRList" << endl;
		abort();
	}
	if (FuncIRList[funcName].back().opcode != "RET") {
		IRNode temp;
		temp.opcode = "RET";
		temp.operand_1 = "";
		temp.operand_2 = "";
		temp.result = "";
		FuncIRList[funcName].push_back(temp);
	}
}
