...
string_decl:	STRING id ASSIGN str SEMICOLON {
                    //printf("\n$2 %s \n$4 %s\n", $2, $4);
                    AddStringSTEToSymbolTable($2, "STRING", $4, "$L");
                }
		;
...
param_decl:		var_type id {
                            ...
                            AddParamSTEToSymbolTable($2, $<str>1, "null","$P");
                        }
			;
...
func_decl: 		FUNCTION any_type id {
                            ...
                            CreateAndPushSymbolTableToStack($3,$3, $<str>2);
                            CreateLabelAndLinkInIRCode($3);
                        } LEFTPAR param_decl_list RIGHTPAR BEGINN func_body 
			END {
			    /* AddRETIfNotThere() is to add RET to IRCode list if not there */
			    AddRETIfNotThere($3);
			    ControlFlowGraphGeneration($3);
                            PopulateGenAndKillSet($3);
                            LivenessAnalysis($3);
			}
			;
...
call_expr: 		id /*{PushIntoExprDeque($1);}*/ 
                LEFTPAR /*{PushIntoExprDeque("(");}*/ {
                    PushIntoExprDeque("#"); // # is a deliminiter for separating different parameters of a function
                }
                expr_list RIGHTPAR /*{PushIntoExprDeque(")")}*/ {
                           //cout << "CALL expr : " << $1 << endl;
                           GenerateIRCode_CallEXPR($1);
                        }
			;
...
expr_list_tail: 	COMMA {
                        PushIntoExprDeque("#"); // # is a deliminiter for separating different parameters of a function
                    }
                    expr {GenerateIRCode_Param_CallEXPR();} expr_list_tail 
			| /*empty*/
			;
...
if_stmt: 		IF{
                        ...
                        } LEFTPAR{PushIntoExprDeque("(");} cond RIGHTPAR{PushIntoExprDeque(")"); 
                         GenerateIRCode_CondExpr();} decl stmt_list else_part ENDIF {GenerateIRCode_AddLblENDIF();} 
			;
...
else_part: 		ELSE{
			...
			| /*empty {GenerateIRCode_AddLblENDIF();} */ /* Ashiwan - else part can be empty. But we will get endif according to the grammar */
			;
...
while_stmt: 		WHILE {
                         ...
                         GenerateIRCode_CondExpr();} decl aug_stmt_list ENDWHILE {GenerateIRCode_while_loop_and_add_endwhile();}
			;
...
/** Ashiwan step5 take care -continue, break**/
aug_stmt: 		base_stmt 
			| aug_if_stmt 
			| while_stmt 
			| CONTINUE {GenerateIRCode_CONTINUE();} SEMICOLON 
			| BREAK {GenerateIRCode_BREAK();} SEMICOLON
			;
...
aug_if_stmt: 		IF {
                          ...
                          GenerateIRCode_CondExpr();} decl aug_stmt_list aug_else_part ENDIF {GenerateIRCode_AddLblENDIF();}
			;
...
aug_else_part: 		ELSE {
			...
			| /*empty {GenerateIRCode_AddLblENDIF();}  Ashiwan - else part can be empty but we will get end if according to the grammar */
			;
